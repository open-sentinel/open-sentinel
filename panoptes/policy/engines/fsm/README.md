...
The FSM engine is Panoptes' **deterministic** policy engine. It models allowed agent behavior as a finite state machine defined in YAML, then classifies each LLM response to a workflow state, evaluates temporal constraints, and triggers interventions when deviations are detected.
...
```python
from panoptes.policy.engines.fsm import FSMCompiler
...
```python
from panoptes.policy.engines.fsm import InterventionBuilder
