...
class WorkflowContextExtractor:
    ...
    def extract_context(
        ...
        # Check headers
        if resolved_headers:
            if workflow := _get_header(resolved_headers, "x-sentinel-workflow"):
                context["workflow_name"] = workflow
            if state := _get_header(resolved_headers, "x-sentinel-expected-state"):
                context["expected_state"] = state
            disable = _get_header(
                resolved_headers, "x-sentinel-disable-intervention"
            )
        ...
        metadata = data.get("metadata", {})
        if isinstance(metadata, dict):
            if workflow := metadata.get("sentinel_workflow"):
                context["workflow_name"] = workflow
            if state := metadata.get("sentinel_expected_state"):
                context["expected_state"] = state
            if metadata.get("sentinel_disable_intervention"):
                context["disable_intervention"] = True

            # Collect any sentinel_ prefixed metadata
            custom = {}
            for key, value in metadata.items():
                if key.startswith("sentinel_") and key not in [
                    "sentinel_workflow",
                    "sentinel_expected_state",
                    "sentinel_disable_intervention",
                    "sentinel_session_id",
                ]:
...
class ResponseTransformer:
    """
    Transform responses to include Open Sentinel metadata.
    ...
    def add_sentinel_headers(
        ...
        """
        Add Open Sentinel headers to response.
        ...
        if workflow_state:
            headers["x-sentinel-workflow-state"] = workflow_state

        if intervention_applied:
            headers["x-sentinel-intervention"] = intervention_applied

        if violations:
            headers["x-sentinel-violations"] = ",".join(violations)
