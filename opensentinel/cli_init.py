...
Generates osentinel.yaml and optionally a starter policy.yaml so users
...
def run_init(
    ...
    """Run the init flow, writing osentinel.yaml and a starter policy file.
    ...
    if engine_type == "judge":
        if policy_style == "simple":
            config_content = OSNTL_YAML_JUDGE_SIMPLE.format(
                model=model, mode=mode, tracing=tracing,
            )
        elif policy_style == "rubric":
            config_content = OSNTL_YAML_JUDGE_RUBRIC.format(
                model=model, mode=mode, tracing=tracing,
            )
        else:
            # file mode: separate policy.yaml
            config_content = OSNTL_YAML_JUDGE.format(
                model=model, mode=mode, tracing=tracing,
            )
            policy_path = Path("policy.yaml")
            policy_content = STARTER_POLICY_YAML
    else:
        config_content = OSNTL_YAML_FSM.format(tracing=tracing)
    ...
    if policy_path:
        ...
        click.echo(f"  {step}. Edit policy rules in osentinel.yaml")
    step += 1
